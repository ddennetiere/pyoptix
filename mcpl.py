...
import struct
from ctypes import c_uint32, c_uint64, c_int32, c_double


hc = h * c / eV / 1e6  # hc in MeV/m

try:
    def _str(s):
        if hasattr(s, 'encode') and bytes == str:
...
_minpy2 = (2, 6, 6)
_minpy3 = (3, 3, 2)
if pyversion < _minpy2 or ((3, 0, 0) <= pyversion < _minpy3):
...
class MCPLFile:
    ...
    def _loadhdr(self):
        ...
        opt_universalweight = float(
            self._fileread(dtype=np_dtype('f8').newbyteorder(endianness), count=1)[0] if _tmp else 0.0)
        ...
        def readarr():
            array = self._fileread(dtype=np_dtype('u4').newbyteorder(endianness), count=1)
            if len(array) != 1:
                raise MCPLError('Invalid header')
            if array == 0:
                return b''
            cont = self._fileread(dtype='u1', count=array)
            if len(cont) != array:
                raise MCPLError('Invalid header')
    ...
    def dump_hdr(self):
        ...
        def print_datastring(prefix, string, postfix):
            print(prefix, end='')
            _output_bytearray_raw(string)
            print(postfix)
    ...
    def dump_particles(self, limit=10, skip=0):
        ...
        s = "index     pdgcode   ekin[MeV]       x[cm]       y[cm]       z[cm]          ux          uy          uz    " \
            "time[ms] "
        if not opt_uw:
            s += "      weight"
        if opt_pol:
...
class PyOptixMCPLReader:
    """
    Reader of MCPL files for PyOptiX

    Use get_diagram to generate a pyoptix Diagram usable as a source for propagation in the code
    """

    def __init__(self, filename, header=True, particles=False, limit=10, skip=0, verbose=1, **kwargs):
        """
        Opens the MCPL file filename and outputs some of it content if verbose is not 0.

        :param filename: name of the file to be opened
        :type filename: str
        :param header: if verbose is > 0, prints file header
        :type header: bool
        :param particles: if verbose is > 0, prints 'limit' particles
        :type particles: bool
        :param limit: number of particles to be printed if verbose is > 0 and particles is True
        :type limit: int
        :param skip: number of particles to be skipped before printing if verbose is > 0 and particles is True
        :type skip: int
        :param verbose: verbose level at opening file
        :type verbose: int
        :param kwargs: additional keyword to pass to mcpl.dump_file
        :type kwargs: dict
        """
        self.filename = filename
        if verbose:
            self.mcpl_file = dump_file(self.filename, header=header, particles=particles, limit=limit, skip=skip,
                                       **kwargs)
        else:
            self.mcpl_file = dump_file(self.filename, header=False, particles=False)
        self.data = pd.DataFrame(columns=["X", "Y", "Z", "dX", "dY", "dZ", "Lambda"])

    def get_diagram(self, show_first_rays=False, ndim=5):
        """
        Extracts photons (gamma) from MCPL files and returns a Diagram structure for PyOptiX
        :return: diagram
        :rtype: optix.Diagram
        """
        block = self.mcpl_file.read_block()
        while block:
            data_block = pd.DataFrame(np.array([block.x[block.pdgcode == 22]/100,
                                                block.y[block.pdgcode == 22]/100,
                                                block.z[block.pdgcode == 22]/100,
                                                block.ux[block.pdgcode == 22],
                                                block.uy[block.pdgcode == 22],
                                                block.uz[block.pdgcode == 22],
                                                hc / (block.ekin[block.pdgcode == 22])]).T,
                                      columns=["X", "Y", "Z", "dX", "dY", "dZ", "Lambda"])
            self.data = self.data.append(data_block)
            block = self.mcpl_file.read_block()
        if ndim == 5:
            data = self.data.drop(columns=["Z", "dZ"])
        else:
            try:
                assert ndim == 7
                data = self.data
            except AssertionError:
                raise AttributeError("ndim must be either 5 or 7")
        if show_first_rays:
            print(data.head())
        return data

    def plot_diagram(self, display="all", light_xy=False, map_xy=False, light_xxp=False, light_yyp=False, show_spd=True,
                     **kwargs):
        """
        Plots the photon particles stored in the MCPL file
        :return: None
        :rtype: Nonetype
        """
        ...
        if "xy" in display:
            figs.append(plot_spd(datasources["xy"], x_key="X", y_key="Y", light_plot=light_xy, show_map=map_xy,
                                 beamline_name=beamline_name, chain_name=chain_name, oe_name="", **kwargs))
        if "xxp" in display:
            figs.append(
                plot_spd(datasources["xxp"], x_key="X", y_key="dX", light_plot=light_xxp, beamline_name=beamline_name,
                         chain_name=chain_name, oe_name="", **kwargs))
        if "yyp" in display:
            figs.append(
...
class PyOptixMCPLWriter:
    """
    Writer of MCPL files for PyOptiX
    ...
    def __init__(self, filename):
        ...
        self.particles = []
        self._set_header()
        self.particle_struct = ""

    def add_comment(self, comment):
        """
        Adds a comment line to be written in the MCPL file.
        ...
        :rtype: Nonetype
        """
        self.header["number_of_comments"] += 1
        self.header["comments"].append(comment)

    def _add_particle(self, x, y, z, dx, dy, dz, t, ekin, polarisation=(None, None, None), weight=None, pdgcode=None,
                     userflags=None):
        """
        Adds a particle to the particle list to be written in the MCPL file;

        :param x: particle x position in m
        :type x: float
        :param y: particle y position in m
        :type y: float
        :param z: particle z position in m
        :type z: float
        :param dx: particle x direction in rad
        :type dx: float
        :param dy: particle y direction in rad
        :type dy: float
        :param dz: particle z direction in rad
        :type dz: float
        :param t: particle time in s
        :type t: float
        :param ekin: particle kinetic energy in MeV
        :type ekin: float
        :param polarisation: list of 3 floats representing X, Y and Z polarisation
        :type polarisation: list
        :param weight: weight of the particle
        :type weight: float
        :param pdgcode: particle designation code (22= photon)
        :type pdgcode: int
        :param userflags: custom userflags
        :type userflags: int
        :return: None
        :rtype: Nonetype
        """
        self.header["number_of_particle"] += 1
        ux, uy, uz = dx, dy, dz # ux = dx/dz mais dz = 1
        norm_u = np.sqrt(ux**2+uy**2+uz**2)
        ux /= norm_u
        uy /= norm_u
        uz /= norm_u
        if max(abs(ux), abs(uy), abs(uz)) == abs(ux):
            if uz != 0:
                packed_u1 = 1/uz
            else:
                packed_u1 = np.inf
            packed_u2 = uy
            extra_bit = int(np.sign(ux))
        elif max(abs(ux), abs(uy), abs(uz)) == abs(uy):
            packed_u1 = ux
            if uz != 0:
                packed_u2 = 1 / uz
            else:
                packed_u2 = np.inf
            extra_bit = int(np.sign(uy))
        else:
            packed_u1 = ux
            packed_u2 = uy
            extra_bit = int(np.sign(uz))
        part = [x*100, y*100, z*100, packed_u1, packed_u2, extra_bit*ekin, t]
        if self.header["polarization_enabled"] == 1:
            for pol in polarisation[::-1]:
                part.insert(pol, 0)
        if self.header["universal_weight_flag"] == 0:
            part.append(weight)
        if self.header["universal_pdgcode"] == 0:
            part.append(pdgcode)
        if self.header["user_flag_enabled"] == 1:
            part.append(userflags)
        self.particles.append(part)
        # note: length are in cm

    def _set_header(self):
        """
        Sets the MCPL header values pr files created with PyOptiX
        :return: None
        :rtype: Nonetype
        """
        self.header["magic_number"] = 0x4d43504c # MCPL in ASCII
        self.header["file_format"] = "003"
        self.header["endianness"] = "L".encode("ascii")
        self.header["number_of_particle"] = 0
        self.header["number_of_comments"] = 0
        self.header["number_of_blobs"] = 0
        self.header["user_flag_enabled"] = 0  # 0 for disabled, 1 for enable
        self.header["polarization_enabled"] = 0  # 0 for disabled, 1 for enable
        self.header["floating_point_precision"] = 0  # 0 for single precision, 1 for double
        self.header["universal_pdgcode"] = 22  # 0 means each particle has its own pdgcode
        self.header["data_length_per_particle"] = None  # computed in dump_diagram
        self.header["universal_weight_flag"] = 1  # 1 for universal weight, 0 for individual weight
        self.header["universal_weight"] = 1
        self.header["source name"] = "PyOptix MCPL writer [PyOptixMCPLWriter]"  # Source name
        self.header["comments"] = []
        self.header["blob_keys"] = []
        self.header["blob_data"] = []

    def dump_diagram(self, diagram):
        """
        Computes the bytesize per particle and adds particles from diagram to the particle list to be written

        :param diagram: PyOptiX Diagram to be recorded
        :type diagram: PyOptiX.classes.Diagram
        :return: None
        :rtype: Nonetype
        """
        assert isinstance(diagram, pd.DataFrame)
        self.header["data_length_per_particle"] = 0
        if self.header["floating_point_precision"]:
            floating_struct = "f"
            float_size = 4
        else:
            floating_struct = "d"
            float_size = 8
        if self.header["polarization_enabled"] == 1:
            self.particle_struct += f"3{floating_struct}"
            self.header["data_length_per_particle"] += float_size
        self.particle_struct += f"3{floating_struct}3{floating_struct}1{floating_struct}"
        self.header["data_length_per_particle"] += 7 * float_size
        if self.header["universal_weight_flag"] == 0:
            self.particle_struct += f"{floating_struct}"
            self.header["data_length_per_particle"] += float_size
        if self.header["universal_pdgcode"] == 0:
            self.particle_struct += "i"
            self.header["data_length_per_particle"] += 4
            raise AttributeError("Unable to process individual pdgcodes at the moment")
        if self.header["user_flag_enabled"] == 1:
            self.particle_struct += "I"
            self.header["data_length_per_particle"] += 4
        if diagram.shape[1] == 5:
            for x, y, dx, dy, lamda in zip(diagram.X, diagram.Y, diagram.dX, diagram.dY, diagram.Lambda):
                self._add_particle(x, y, 0, dx, dy, 1, 0, hc / lamda)
        elif diagram.shape[1] == 7:
            for x, y, z, dx, dy, dz, lamda in zip(diagram.X, diagram.Y, diagram.Z, diagram.dX, diagram.dY, diagram.dZ,
                                                  diagram.Lambda):
                self._add_particle(x, y, z, dx, dy, dz, 0, hc / lamda)
        else:
            raise AttributeError("unable to interpret diagram")

    def write_to_file(self):
        """
        Writes the MCPL file at adress provided in contructor

        :return: None
        :rtype: Nonetype
        """
        with open(self.filename, "wb") as fo:
            fo.write(struct.pack('>I', self.header["magic_number"]))
            fo.write(struct.pack('c', self.header["file_format"][0].encode("ascii")))
            fo.write(struct.pack('c', self.header["file_format"][1].encode("ascii")))
            fo.write(struct.pack('c', self.header["file_format"][2].encode("ascii")))
            fo.write(struct.pack('c', self.header["endianness"]))
            fo.write(struct.pack('Q', self.header["number_of_particle"]))
            fo.write(struct.pack('I', self.header["number_of_comments"]))
            fo.write(struct.pack('I', self.header["number_of_blobs"]))
            fo.write(struct.pack('I', self.header["user_flag_enabled"]))
            fo.write(struct.pack('I', self.header["polarization_enabled"]))
            fo.write(struct.pack('I', self.header["floating_point_precision"]))
            fo.write(struct.pack('i', self.header["universal_pdgcode"]))
            fo.write(struct.pack('I', self.header["data_length_per_particle"]))
            fo.write(struct.pack('I', self.header["universal_weight_flag"]))
            if self.header["universal_weight_flag"]:
                fo.write(struct.pack('d', self.header["universal_weight"]))
            fo.write(struct.pack('I', len(self.header["source name"])))
            fo.write(struct.pack(f'{len(self.header["source name"])}c',
                                 *[char.encode("ascii") for char in self.header["source name"]]))
            for comment in self.header["comments"]:
                fo.write(struct.pack('I', len(comment)))
                fo.write(struct.pack(f'{len(comment)}c',
                                     *[char.encode("ascii") for char in comment]))
            for particle in self.particles:
                # print(self.particle_struct, *particle)
                fo.write(struct.pack(self.particle_struct, *particle))
...
def convert2ascii(mcplfile, outfile):
    ...
    fmtstr = "%5i %11i %23.18g %23.18g %23.18g %23.18g %23.18g %23.18g %23.18g %23.18g %23.18g %23.18g %23.18g " \
             "%23.18g 0x%08x\n "
    for idx, p in enumerate(fin.particles):
        fout.write(fmtstr % (
            idx, p.pdgcode, p.ekin, p.x, p.y, p.z, p.ux, p.uy, p.uz, p.time, p.weight, p.polx, p.poly, p.polz,
            p.userflags))
...
def app_pymcpltool(argv=None):
    ...
    for a in args:
        if a.startswith(str('--')):
            ...
            filelist += [a]
    number_dumpopts = sum(
        1 for e in (opt_justhead, opt_nohead, opt_limit is not None, opt_skip is not None, opt_blobkey) if e)
...
def _pdg_database(pdgcode):
    ...
    if pdgcode > 0 and pdgcode // 100000000 == 10:
        first_part = pdgcode % 10
        pdgcode //= 10
        second_part = pdgcode % 1000
        pdgcode //= 1000
        third_part = pdgcode % 1000
        pdgcode //= 1000
        fourth_part = pdgcode % 10
        pdgcode //= 10
        if pdgcode == 10 and third_part > 0 and second_part > 0:
            if fourth_part == 0 and first_part == 0 and third_part < len(_db_elem) + 1:
                return '%s%i' % (_db_elem[third_part - 1], second_part)
            s = 'ion(Z=%i,A=%i' % (third_part, second_part)
            if fourth_part:
                s += ',L=%i' % fourth_part
            if first_part:
                s += ',I=%i' % first_part
...
def _unique_count(a, weights=None):
    """returns (unique,count) where unique is an array of sorted unique values in a, and count is the corresponding
    frequency counts """
...
class _StatCollector:
    ...
    def dump(self):
        for k in self.__dumporder:
            print("%s : %s" % (
                k.ljust(8), '%g' % self.__statcalc[k]() if self.__sumw > 0.0 or k == 'integral' else 'n/a'))

    def summarise(self):
        return ', '.join(
            "%s=%s" % (k, '%g' % self.__statcalc[k]() if self.__sumw > 0.0 or k == 'integral' else 'n/a') for k in
            self.__dumporder)
